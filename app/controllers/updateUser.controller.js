...
exports.create = async (req, res) => {
  ...
  let userId = req.body.userId, cardNumber = req.body.cardNumber;
  let itemId = 0;

  const fileUrl = "https://api.monday.com/v2/file";

  const changeColumnQuery = 'mutation ($boardId:Int!, $itemId: Int!, $columnId: String!, $columnValue: String!) {change_simple_column_value (board_id: $boardId, item_id: $itemId, column_id: $columnId, value: $columnValue) {id}}';
  const fileQuery = 'mutation ($file: File!, $itemId: Int!) { add_file_to_column (file: $file, item_id: $itemId, column_id: "files") { id } }';
  ...
  const moveItemToGroupMutation = 'mutation ($itemId: Int!, $groupId: String!) { move_item_to_group (item_id: $itemId, group_id: $groupId) { id }}'
  ...
  async function getItem() {
    return new Promise(function(resolve, reject) {
      sql.query("SELECT mondayId FROM users where userId = ?", [userId], (err, res) => {  
  ...
  const updateCRM = async() => {
    try {
      ...
      const response = await axios.post("https://api.monday.com/v2/", mutationData, {
      headers: {
      ...getHeaders()
      }
      });
      ...
      if(itemId) {
        ...
        const {payload, boundary} = await createfilePayload(fileQuery, variables);
        //CRM IMPORT FILE
        try {
        ...
        //CRM _ CHANGE ROW
        try {
          const columndResponse = await axios.post("https://api.monday.com/v2/", mutationDataMoveItem, {
          headers: {
          ...getHeaders()
        }
        });
